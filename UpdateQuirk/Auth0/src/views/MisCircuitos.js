...
const MisCircuitos = () => {
  ...
  const handleTranslate = async (url, nombre, url_desplegada) => {
    ...
    try {
      ...
      const awsResponse = await axios.post(
        "http://ec2-54-155-193-167.eu-west-1.compute.amazonaws.com:8083/code/aws",
        {
          url: url,
        },
        {
      ...
      const ibmResponse = await axios.post(
        "http://ec2-54-155-193-167.eu-west-1.compute.amazonaws.com:8083/code/ibm",
        {
          url: url,
        },
        {
  ...
  // Función para abrir la página de depuración en la misma pestaña
  const handleDebugClick = (url, id) => {
    const [baseUrl, hash] = url.split("#");
    const separator = baseUrl.includes("?") ? "&" : "?";
    const newUrl = `${baseUrl}${separator}debug=true&id=${id}${hash ? "#" + hash : ""}`;

    // Opción para abrir en la misma pestaña
    window.location.href = newUrl;

    // Opción para abrir en otra pestaña
                ...
                <div className="mis-circuitos-circuito-buttons">
                  ...
                  <div className="mis-circuitos-circuito-buttons-middle">
                    ...
                    <button
                      disabled={isEditingName || isEditingOrEjecting}
                      className="mis-circuitos-icon-button"
                      onClick={() => handleDebugClick(circuito.url, circuito.id)}
                    >
                      <img
                        src={debugIcon}
                        alt="Debug"
        ...
        <Modal isOpen={modal} toggle={toggle} centered={true}>
          ...
          <ModalFooter>
            <div class="buttons-modal">
              ...
              <div className="centrador">
                <Button color="success" onClick={comparitrCircuito}>
                  Añadir
                </Button>
              </div>
            </div>
